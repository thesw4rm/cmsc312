...
csema_t full_sem;  /* when 0, buffer is full */
csema_t empty_sem; /* when 0, buffer is empty. Kind of
...
void *producer(void *thread_n) {
    // printf("PRODUCER STARTED %i\n", full_sem.csem_val);
    int thread_numb = *(int *)thread_n;
    buffer_t value;
    int i=0;
    while (i++ < PRODUCER_LOOPS) {
        sleep(rand() % 10); // sleep(rand() % 10)
        value = rand() % 100;
        csema_wait(&full_sem, 1, 0); // sem=0: wait. sem>0: go and decrement it
                // printf("OLLO\n");
        ...
        csema_post(&empty_sem, 1, 1); // post (increment) emptybuffer semaphore
...
void *consumer(void *thread_n) {
   // // printf("CONSUMER STARTED %i\n", full_sem.csem_val);
    int thread_numb = *(int *)thread_n;
    buffer_t value;
    int i=0;
    while (i++ < PRODUCER_LOOPS) {
        csema_wait(&empty_sem, 0, 1);
                // printf("OLLOCONS\n");
        ...
        pthread_mutex_unlock(&buffer_mutex);
        csema_post(&full_sem, 0, 0); // post (increment) fullbuffer semaphore
...
int main(int argc, int **argv) {
    ...
    csema_init(&full_sem, // sem_t *sem, // int pshared. 0 = shared between threads of process,  1 = shared between processes
             SIZE); // unsigned int value. Initial value
    csema_init(&empty_sem,
             0);
    ...
    csema_destroy(&full_sem);
    csema_destroy(&empty_sem);
