#include <rpc/rpc.h>
#include "avg.h"
#include <stdio.h>
#include <string.h>
/* run locally on 'server' called by a remote client. */
static double sum_avg;
static double median;
static char *nums;
void sw(double *x, double *y){
  double t = *x;
  *x = *y;
  *y = t;
}

int part(double *arr, int s, int e, const int a){
  // set pivot as last element
  int piv = arr[e];
  int i = s - 1; // Get smaller element
  int j;
  for(j = s; j <= e; j++){
    if(a != 0){
      if(arr[j] < piv){
        ++i;
        sw(&arr[i], &arr[j]);
      }
    }
    else{
      if(arr[j] > piv){
        ++i;
        sw(&arr[i], &arr[j]);
      }
    }
  }
  sw(&arr[i+1], &arr[e]);
  return i+1; // Because j runs through upper ind elements and i runs through lower ind elements, "i" will at this point be the middle point of the array
}

double *qs(double *arr, int s, int e, const int a){
  if(s < e){
    int p = part(arr, s, e, a);
    qs(arr, s, p - 1, a);
    qs(arr, p+1, e, a);
  }
}



/* 
 * routine notice the _1 the version number 
 * notice the client handle, not sued here but needs to be 
 * a parameter 
 */
char **average_1(input_data *input, CLIENT *client) 
  {

  /* input is paramters were marshalled by genrated routine */
  /*  a pointer to a double, is set to begining of data array  */
  double *dp = input->input_data.input_data_val;

  u_int i;
  // sum_avg = 0;

  // /* iterate until end of number of times (data_len) */
  // for( i = 1; i <= input->input_data.input_data_len; i++ )
  //   {
  //   sum_avg = sum_avg + dp[i];  /* add what ptrs points  to ( '*' gets content ) */
  
  //   }
  printf("%f\n", dp[0]);

  // sum_avg = sum_avg / input->input_data.input_data_len;
  qs(dp, 1, input->input_data.input_data_len - 1, dp[0]);
  //memcpy(num_arr, &dp[1], sizeof(double) * input->input_data.input_data_len);
  nums = malloc(100);
  memset(nums, 0, 100);
  int ind;
  ind = 0;
  int g;
  char temp[10];
  for(g = 1; g < input->input_data.input_data_len; g++){
      snprintf(temp, 10, "%f", dp[g]);
      int len = strlen(temp);
      memcpy(&nums[ind], temp, len);
      ind += len;
      nums[ind] = ' ';
      ind+=1;
      //nums[ind] = ' ';
      //ind++;
      printf("%s\n", nums);
  }
  // int g;
  // for(g = 1; g < input->input_data.input_data_len; g++){
  //   printf("%f\n", dp[g]);
  //   snprintf(temp_double, 9, "%f", dp[g]);
  //   strncpy(&num_arr[ind], temp_double, 10);
  //   ind += strlen(temp_double);
  //   num_arr[ind] = ' ';
  //   ++ind;
  //   memset(temp_double, 0, 10);
  // }
  // free(temp_double);
  return( &nums );
}



/* 
 * server stub 'average_1_svc function handle called in avg_svc that was
 * generated by rpcgen 
 * FYI:
 *   result = (*local)((char *)&argument, rqstp);
 *   where local is (char *(*)(char *, struct svc_req *)) average_1_svc;
 */
char **average_1_svc(input_data *input, struct svc_req *svc) 
  {
  CLIENT *client;
  return( average_1( input, client) );
  }

